...
int fimc_is_spi_s_pin(struct fimc_is_spi *spi, bool on)
{
	...
	if (on) {
		/* FW usage: auto mode CS pin */
		s = (struct pinctrl_state *)spi->pin_ssn_fn;
		ret = pinctrl_select_state(spi->pinctrl, s);
		if (ret < 0) {
			pr_err("pinctrl_select_state is fail(%d)\n", ret);
			return ret;
		}
	} else {
		/* Host usage: output mode CS pin */
		s = (struct pinctrl_state *)spi->pin_ssn_out;
		ret = pinctrl_select_state(spi->pinctrl, s);
		if (ret < 0) {
			pr_err("pinctrl_select_state is fail(%d)\n", ret);
			return ret;
