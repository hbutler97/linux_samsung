...
int fimc_is_sensor_gpio_on(struct fimc_is_device_sensor *device)
{
	...
	module = NULL;
	gpio_scneario = GPIO_SCENARIO_ON;
	core = device->private_data;
	...
	if (!test_and_set_bit(FIMC_IS_MODULE_GPIO_ON, &module->state)) {
		...
		if (module->ext.preprocessor_con.product_name != PREPROCESSOR_NAME_NOTHING) {
			ret = fimc_is_vender_gpio_on_sel(vender, scenario, &gpio_scneario);
			if (ret) {
				clear_bit(FIMC_IS_MODULE_GPIO_ON, &module->state);
				merr("fimc_is_vender_gpio_on_sel is fail(%d)", device, ret);
				goto p_err;
			}
		}
		...
		ret = pdata->gpio_cfg(module->dev, scenario, gpio_scneario);
		if (ret) {
			clear_bit(FIMC_IS_MODULE_GPIO_ON, &module->state);
		...
		if (module->ext.preprocessor_con.product_name != PREPROCESSOR_NAME_NOTHING) {
			ret = fimc_is_vender_gpio_on(vender, scenario, gpio_scneario);
			if (ret) {
				merr("fimc_is_vender_gpio_on is fail(%d)", device, ret);
				goto p_err;
			}
		}
	}
...
int fimc_is_sensor_gpio_off(struct fimc_is_device_sensor *device)
{
	...
	module = NULL;
	gpio_scneario = GPIO_SCENARIO_OFF;
	core = device->private_data;
	scenario = device->pdata->scenario;
	vender = &core->vender;
	...
	if (test_and_clear_bit(FIMC_IS_MODULE_GPIO_ON, &module->state)) {
		...
		if (module->ext.preprocessor_con.product_name != PREPROCESSOR_NAME_NOTHING) {
			ret = fimc_is_vender_gpio_off_sel(vender, scenario, &gpio_scneario);
			if (ret) {
				set_bit(FIMC_IS_MODULE_GPIO_ON, &module->state);
				merr("fimc_is_vender_gpio_off_sel is fail(%d)", device, ret);
				goto p_err;
			}
		}
		...
		if (module->ext.preprocessor_con.product_name != PREPROCESSOR_NAME_NOTHING) {
			ret = fimc_is_vender_gpio_off(vender, scenario, gpio_scneario);
			if (ret) {
				merr("fimc_is_vender_gpio_off is fail(%d)", device, ret);
				goto p_err;
			}
		}
	}
	...
	u32 scenario)
{
	...
	for (sensor_index = 0; sensor_index < SENSOR_MAX_ENUM; sensor_index++) {
		if (module_enum[sensor_index].sensor_id == input) {
			...
			/*
			 * If it is not normal scenario and is not virtual sensor,
			 * try to find proper sensor module which has a i2c client
			 */
			if (scenario == SENSOR_SCENARIO_VIRTUAL)
				break;
			else if (scenario != SENSOR_SCENARIO_NORMAL && module->client == NULL)
				continue;
			else
				break;
		}
