...
static const struct drm_ioctl_desc exynos_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(EXYNOS_GEM_OFFSET,
			  exynos_drm_gem_offset_ioctl, DRM_UNLOCKED),
				       ...
				       int count)
{
	...
	for (i = 0; i < count; ++i) {
		...
		DRM_INFO("platform_driver_register:%s[%d]\n", drv[i]->driver.name, ret);
		if (!ret)
			continue;
