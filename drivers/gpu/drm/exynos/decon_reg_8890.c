/* linux/drivers/gpu/drm/exynos-hq/decon_reg_8890.c
 *
	...
	u32 dsi_idx, struct decon_mode_info *psr)
{
	...
	if (!disp_ss_regs) {
		DRM_ERROR("decon%d disp_ss_regs is not mapped\n", id);
		return;
	}
	...
	DRM_DEBUG_KMS("Display Configuration(DISP_CFG) value is 0x%x\n", val);
}
...
int decon_reg_reset(u32 id)
{
	...
	if (!tries) {
		DRM_ERROR("decon%d failed to reset Decon\n", id);
		return -EBUSY;
	}
...
void decon_reg_configure_lcd(u32 id, enum decon_dsi_mode dsi_mode, struct decon_lcd *lcd_info)
{
	...
	if (lcd_info->mic_enabled) {
		...
		if (lcd_info->dsc_cnt == 1)
			...
			DRM_ERROR("not supported data path for %d DSC of decon%d",
					lcd_info->dsc_cnt, id);
	...
	DRM_DEBUG_KMS("%s: decon%d, enhance path(0x%x), data path(0x%x)\n",
...
int decon_reg_wait_linecnt_is_zero_timeout(u32 id, int dsi_idx, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d wait timeout linecount is zero(%u)\n", id, linecnt);
		return -EBUSY;
	}
...
int decon_reg_wait_idle_status_timeout(u32 id, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d wait timeout decon idle status(%u)\n", id, status);
		return -EBUSY;
	}
...
int decon_reg_wait_run_is_off_timeout(u32 id, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d wait timeout decon run is shut-off(%u)\n", id, status);
		return -EBUSY;
	}
...
int decon_reg_wait_run_status_timeout(u32 id, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d wait timeout decon run status(%u)\n", id, status);
		return -EBUSY;
	}
...
u32 decon_get_win_channel(u32 id, u32 win_idx, enum decon_idma_type type)
{
	...
	case IDMA_VGR1:
		...
		break;
	default:
		DRM_ERROR("decon%d channel(0x%x) is not valid\n", id, type);
		return -EINVAL;
	}
...
int decon_reg_wait_for_update_timeout(u32 id, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d timeout of updating decon registers\n", id);
		return -EBUSY;
	}
...
int decon_reg_wait_for_window_update_timeout(u32 id, u32 win_idx, unsigned long timeout)
{
	...
	if (!cnt) {
		DRM_ERROR("decon%d timeout of updating decon window registers\n", id);
		return -EBUSY;
	}
...
void dsc_reg_set_pps_size(u32 encoder_id, struct decon_lcd *lcd_info)
{
	...
	if (lcd_info->dsc_cnt == 1) {
		...
		picture_w = lcd_info->xres;
		DRM_ERROR("DSC max count is %d\n", lcd_info->dsc_cnt);
	}

	DRM_DEBUG_KMS("DSC%d: picture w(%d) h(%d)\n", encoder_id, picture_w, picture_h);
	DRM_DEBUG_KMS("slice w(%d) h(%d), chunk(%d)\n",
...
int dsc_reg_init(u32 id, enum decon_dsi_mode dsi_mode, struct decon_lcd *lcd_info)
{
	...
	DRM_DEBUG_KMS("SPLITTER size: w(%d), h(%d)\n", w, h);
	decon_reg_get_frame_fifo_size(id, 0, &w, &h);
	DRM_DEBUG_KMS("FIFO0 size: w(%d), h(%d)\n", w, h);
	decon_reg_get_frame_fifo_size(id, 1, &w, &h);
	DRM_DEBUG_KMS("FIFO1 size: w(%d), h(%d)\n", w, h);
	decon_reg_get_dispif_size(id, 0, &w, &h);
	DRM_DEBUG_KMS("DISPIF0 size: w(%d), h(%d)\n", w, h);
...
int decon_reg_init(u32 id, u32 dsi_idx, struct decon_param *p)
{
	...
	if(id==0) decon_reg_reset(id);	/*DISPIF3_TEST*/
...
int exynos_decon_reg_get_interrupt_and_clear(u32 id)
{
	...
	if (val & INTERRUPT_RESOURCE_CONFLICT_INT_EN) {
		...
		printk("decon%d RESOURCE_OCCUPANCY_INFO_0: 0x%x, INFO_1: 0x%x\n",
			id,
			decon_read(id, RESOURCE_OCCUPANCY_INFO_0),
			decon_read(id, RESOURCE_OCCUPANCY_INFO_1));
		printk("decon%d RESOURCE_SEL_0: 0x%x, SEL_1: 0x%x, INDUCER: 0x%x\n",
			id,
...
const unsigned long decon_clocks_table[][CLK_ID_MAX] = {
	...
	{    75,   168,   400,    66,        75, 1080 * 1440,     MIC_COMP_BYPASS,         0},
...
{
	int i = sizeof(decon_clocks_table) / sizeof(decon_clocks_table[0]);
	...
	DRM_DEBUG_KMS("%s: VCLK %ld ECLK %ld ACLK %ld PCLK %ld DPLL %ld\n",
		__func__,
