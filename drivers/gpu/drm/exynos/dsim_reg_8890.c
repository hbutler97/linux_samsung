/* linux/drivers/gpu/drm/exynos-hq/dsim_reg_8890.c
 *
...
u32 dsim_reg_wait_lane_stop_state(u32 id)
{
	...
	if (!cnt) {
		pr_err("wait timeout DSI Master is not stop state.\n");
		pr_err("check initialization process.\n");
		return -EBUSY;
	}
...
void dsim_reg_set_porch(u32 id, struct decon_lcd *lcd)
{
	...
	if (lcd->mode == DECON_VIDEO_MODE) {
		...
		pr_err("This DDI is not MIPI interface.\n");
		return;
	}
}
...
void dsim_reg_set_config(u32 id, struct decon_lcd *lcd_info, u32 data_lane_cnt)
{
	...
	if (lcd_info->mode == DECON_VIDEO_MODE) {
		...
		if (lcd_info->ddi_type == TYPE_OF_SM_DDI)
...
void dsim_reg_set_size_of_slice(u32 id, struct decon_lcd *lcd_info)
{
	...
	if (lcd_info->dsc_slice_num == 4) {
		...
		pr_err("not supported slice mode. dsc(%d), slice(%d)\n",
...
void dsim_reg_print_size_of_slice(u32 id)
{
	...
	pr_debug("dsim%d: slice0 w(%d), slice1 w(%d), slice2 w(%d), slice3(%d)\n",
...
int dsim_reg_wait_hs_clk_ready(u32 id)
{
	...
	if (!cnt) {
		pr_err("DSI Master is not HS state.\n");
		return -EBUSY;
	}
...
int dsim_reg_wait_for_hdr_fifo_empty(u32 id)
{
	...
	do {
		state = dsim_reg_header_fifo_is_empty(id);
		cnt--;
		udelay(10);
	} while (!state && cnt);
...
int dsim_reg_wait_enter_ulps_state(u32 id, u32 lanes)
{
	...
	if (!cnt) {
		pr_err("DSI Master is not ULPS state.\n");
		return -EBUSY;
	} else {
		pr_debug("DSI Master is ULPS state.\n");
	}
...
int dsim_reg_wait_exit_ulps_state(u32 id)
{
	...
	if (!cnt) {
		pr_err("DSI Master is not stop state.\n");
		return -EBUSY;
	} else {
		pr_debug("DSI Master is stop state.\n");
	}
...
static int dsim_reg_get_dphy_timing(u32 hs_clk, u32 esc_clk, struct dphy_timing_value *t)
{
	...
	if (!i) {
		pr_err("%u Mhz hs clock can't find proper dphy timing values\n", hs_clk);
		return -EINVAL;
	}

	if ((esc_clk > 20) || (esc_clk < 7)) {
		pr_err("%u Mhz cann't be used as escape clock\n", esc_clk);
		return -EINVAL;
	}
...
int dsim_reg_init(u32 id, struct decon_lcd *lcd_info, u32 data_lane_cnt, struct dsim_clks *clks)
{
	...
	if (dsim_read_mask(id, DSIM_CLKCTRL, DSIM_CLKCTRL_TX_REQUEST_HSCLK)) {
		pr_info("dsim%d is probed with LCD ON UBOOT\n", id);
	...
	pr_debug("byte clock is %u MHz\n", clks->byte_clk);

	/* requeseted escape clock */
	pr_debug("requested escape clock %u MHz\n", clks->esc_clk);
	...
	pr_debug("escape clock divider is 0x%x\n", esc_div);
	pr_debug("escape clock is %u MHz\n", clks->esc_clk);
	...
	if (lcd_info->dsc_enabled) {
		pr_debug("%s: dsc configuration is set\n", __func__);
		...
		pr_debug("dsim%d: number of DSC slice(%d)\n", id, num_of_slice);
...
int dsim_reg_set_clocks(u32 id, struct dsim_clks *clks, struct stdphy_pms *dphy_pms, u32 en)
{
	...
	if (en) {
		 ...
		 */
		if (dsim_reg_is_pll_stable(id)) {
			pr_info("dsim%d DPHY PLL is already stabled\n", id);
			dsim_reg_set_standby(id, 0);
			return -EBUSY;
		}
...
int dsim_reg_rx_err_handler(u32 id, u32 rx_fifo)
{
	...
	if ((err_bit & MIPI_DSI_ERR_BIT_MASK) == 0) {
		pr_debug("dsim%d, Non error reporting format (rx_fifo=0x%x)\n",
				id, rx_fifo);
		return ret;
	}
...
int dsim_reg_set_ulps_by_ddi(u32 id, u32 ddi_type, u32 lanes, u32 en)
{
	...
	case TYPE_OF_MAGNA_DDI:
		pr_err("This ddi(%d) doesn't support ULPS\n", ddi_type);
		ret = -EINVAL;
		break;
	...
	if (ret < 0)
		pr_err("%s: failed to %s ULPS", __func__,
				en ? "enter" : "exit");
