/* linux/drivers/gpu/drm/exynos-hq/exynos_drm_decon.h
 *
 * Copyright (c) 2016 Samsung Electronics Co., Ltd.
 *		http://www.samsung.com
 *
 * Header file for Samsung Exynos DECON driver.
 *
...
struct decon_context {
	...
	struct exynos_drm_plane		planes[MAX_WINDOW_CNT];
	struct exynos_drm_plane_config	configs[MAX_WINDOW_CNT];
	void __iomem			*regs;
...
extern struct decon_context *decon_drvdata[MAX_DECON_CNT];
extern struct decon_context *decon_t_int_ctx;

static inline struct decon_context *get_decon_drvdata(u32 id)
{
...
{
	struct decon_context *decon = get_decon_drvdata(id);
	return readl(decon->regs + reg_id);
}

static inline u32 decon_cmu_read(u32 id, u32 reg_id)
{
	struct decon_context *decon = get_decon_drvdata(id);
	return readl(decon->cmu_regs + reg_id);
}
...
static inline void decon_cmu_write(u32 id, u32 reg_id, u32 val)
{
	struct decon_context *decon = get_decon_drvdata(id);
	writel(val, decon->cmu_regs + reg_id);
}
