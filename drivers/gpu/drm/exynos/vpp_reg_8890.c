/* linux/drivers/gpu/drm/exynos-hq/vpp_reg_8890.c
 *
 * Copyright (c) 2016 Samsung Electronics Co., Ltd.
 *		http://www.samsung.com
 *
 * SFR access functions for Exynos VPP driver
 *
...
int vpp_reg_wait_op_status(u32 id)
{
	...
	DRM_ERROR("timeout op_status to idle\n");
...
void vpp_reg_wait_pingpong_clear(u32 id)
{
	...
	DRM_ERROR("timeout of VPP(%d) pingpong_clear\n", id);
}
...
void vpp_reg_set_format(u32 id, struct vpp_params_info *p)
{
	...
	vpp_reg_set_in_afbc_en(id, p->is_comp);
}
...
int vpp_reg_set_rotation(u32 id, struct vpp_params_info *p)
{
	vpp_write_mask(id, VG_IN_CON, p->is_rot << 8, VG_IN_CON_IN_ROTATION_MASK);
	if (p->is_rot > 0)
...
void vpp_reg_set_scale_ratio(u32 id, struct vpp_params_info *p)
{
	vpp_write(id, VG_H_RATIO, p->h_ratio);
	vpp_reg_set_h_coef(id, p->h_ratio);
	vpp_write(id, VG_V_RATIO, p->v_ratio);
	vpp_reg_set_v_coef(id, p->v_ratio);

	DRM_DEBUG_KMS("h_ratio : %#x, v_ratio : %#x\n", p->h_ratio, p->v_ratio);
}

void vpp_reg_set_buf_addr(u32 id, struct vpp_params_info *p)
{
	DRM_DEBUG_KMS("y : %llu, cb : %llu, cr : %llu\n",
			p->addr[0], p->addr[1], p->addr[2]);

	vpp_write(id, VG_BASE_ADDR_Y(0), p->addr[0]);
	/* When processing the AFBC data, BASE_ADDR_Y and
	 * BASE_ADDR_CB should be set to the same address.
	 */
	if (p->is_comp == 0)
		vpp_write(id, VG_BASE_ADDR_CB(0), p->addr[1]);
	else
		vpp_write(id, VG_BASE_ADDR_CB(0), p->addr[0]);

	vpp_write(id, VG_PINGPONG_UPDATE, VG_ADDR_PINGPONG_UPDATE);
}

void vpp_reg_set_size(u32 id, struct vpp_params_info *p)
{
	u32 cfg = 0;
	...
	vpp_write(id, VG_SMART_IF_PIXEL_NUM, p->dst.w * p->dst.h);
}

void vpp_reg_set_block_area(u32 id, struct vpp_params_info *p)
{
	u32 cfg = 0;
	...
	DRM_DEBUG_KMS("block x : %d, y : %d, w : %d, h : %d\n",
...
void vpp_reg_set_rgb_type(u32 id, u32 type)
{
	...
	case BT_709_WIDE:
		...
		break;
	default:
		DRM_ERROR("Unsupported CSC Equation\n");
	}
...
int vpp_reg_deinit(u32 id, bool reset)
{
	...
	if (!reset) {
		ret = vpp_reg_wait_op_status(id);
		if (ret) {
			DRM_ERROR("dpp-%d is working\n", id);
			return -EBUSY;
		}
	}

	vpp_irq = vpp_reg_get_irq_status(id);
	vpp_reg_clear_irq(id, vpp_irq);
...
static void vpp_reg_set_color_fill_en(u32 id, u32 val)
{
	vpp_write_mask(id, IDMA_COLOR_FILL_EN,
			val << F_COLOR_FILL_EN, 0x1 << F_COLOR_FILL_EN);
}

static void vpp_reg_set_fill_color(u32 id, u32 argb)
{
	vpp_write(id, IDMA_FILL_COLOR, argb);
}

void vpp_reg_set_color_fill_mode(u32 id, u32 en, u32 argb)
{
	vpp_reg_set_color_fill_en(id, en);
	vpp_reg_set_fill_color(id, argb);
}

//#define COLOR_FILL_MODE
#define COLOR_FILL_VALUE 0x00ff00
void vpp_reg_configure_params(u32 id, struct vpp_params_info *p)
{
#ifdef COLOR_FILL_MODE
	if (p->addr[0])
		vpp_reg_set_color_fill_mode(id, 1, COLOR_FILL_VALUE);
#endif
	vpp_reg_wait_pingpong_clear(id);
	vpp_reg_set_rgb_type(id, p->eq_mode);
	vpp_reg_set_scale_ratio(id, p);
	vpp_reg_set_size(id, p);
	vpp_reg_set_rotation(id, p);
	vpp_reg_set_buf_addr(id, p);
	vpp_reg_set_block_area(id, p);
	vpp_reg_set_format(id, p);
}
