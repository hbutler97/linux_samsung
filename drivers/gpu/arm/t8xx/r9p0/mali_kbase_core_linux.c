...
/**
 * kbase_show_gpu_meminfo - Show callback for the gpu used memory info sysfs entry.
 * @dev: The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf: The output buffer to receive the GPU used memory information.
 *
 * This function is called to get a description of the present Mali
 * GPU via the gpuinfo sysfs entry.  This includes the GPU family, the
 * number of cores, the hardware version and the raw product id.  For
 ...
 * Return: The number of bytes output to buf.
 */
static ssize_t kbase_show_gpu_meminfo(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct list_head *entry;
	...
	list_for_each(entry, kbdev_list) {
		...
		mutex_unlock(&kbdev->kctx_list_lock);
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, "===========================================================\n");
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, " %16s  %18s  %20s\n", \
				"dev name", \
				"total used kbytes", \
				"total shrink kbytes");
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, "-----------------------------------------------------------\n");
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, " %16s  %18u  %20zu\n", \
				kbdev->devname, \
				atomic_read(&(kbdev->memdev.used_pages))*4, \
				free_size*4);
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, "===========================================================\n\n");
		ret += scnprintf(buf+ret, PAGE_SIZE-ret, "%28s     %20s  %16s  %12s\n", \
				"context name", \
				"context addr", \
				"used kbytes", \
				"shrink kbytes");
		...
		list_for_each_entry(element, &kbdev->kctx_list, link) {
			...
			* opened on this device */
			...
			spin_unlock(&(element->kctx->mem_pool.pool_lock));
			ret += scnprintf(buf+ret, PAGE_SIZE-ret, "  (%24s), %s-0x%p    %12u  %10zu\n", \
					element->kctx->name, \
					"kctx", \
					element->kctx, \
					atomic_read(&(element->kctx->used_pages))*4,
					each_free_size*4 );
		}
		mutex_unlock(&kbdev->kctx_list_lock);
	}
...
static DEVICE_ATTR(gpu_meminfo, S_IRUGO, kbase_show_gpu_meminfo, NULL);
