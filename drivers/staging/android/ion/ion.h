 ...
 * @client:	the client
 * @handle:	the handle
 * @prv:	the argument to the @release function
 * @release:	release function called when the dmabuf is released
 */
struct dma_buf *ion_share_dma_buf_callback(struct ion_client *client,
					struct ion_handle *handle,
					void *prv, void (*release)(void *prv));
/**
 * ion_share_dma_buf() - share buffer as dma-buf
 * @client:	the client
 * @handle:	the handle
...
static inline struct dma_buf *ion_share_dma_buf(struct ion_client *client,
						struct ion_handle *handle)
{
	return ion_share_dma_buf_callback(client, handle, NULL, NULL);
}
...
/**
 * __ion_import_dma_buf() - given an dma-buf from the ion exporter get handle
 * @client:	the client
 * @dmabuf:	the dma-buf object
 *
 * Given an dma-buf object that was allocated through ion via ion_share_dma_buf,
 * import that object and return a handle representing it.  If a dma-buf from
 * another exporter is passed in this function will return ERR_PTR(-EINVAL)
 */
