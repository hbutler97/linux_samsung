...
static int dwc3_exynos_probe(struct platform_device *pdev)
{
	...
	if (exynos->drv_data->mux_sel) {
		/* init mux */
		exynos->usb_ss_sel = of_get_named_gpio(dev->of_node,
							"samsung,usb_ss_sel", 0);
		if (!gpio_is_valid(exynos->usb_ss_sel))
			dev_info(dev, "usb_ss_sel is not available\n");

		exynos->usb_hs_sel = of_get_named_gpio(dev->of_node,
							"samsung,usb_hs_sel", 0);
		if (!gpio_is_valid(exynos->usb_hs_sel))
			dev_info(dev, "usb_hs_sel is not available\n");
		...
		/* mux sel */
		if (gpio_is_valid(exynos->usb_ss_sel)) {
			ret = devm_gpio_request_one(dev, exynos->usb_ss_sel,
							GPIOF_OUT_INIT_LOW, "dwc3_ss_sel");
			if (ret) {
				dev_err(exynos->dev, "failed to request dwc3 ss_sel gpio");
				return ret;
			}

			gpio_set_value(exynos->usb_ss_sel, 0);
		}
		if (gpio_is_valid(exynos->usb_hs_sel)) {
			ret = devm_gpio_request_one(dev, exynos->usb_hs_sel,
							GPIOF_OUT_INIT_LOW, "dwc3_hs_sel");
			if (ret) {
				dev_err(exynos->dev, "failed to request dwc3 hs_sel gpio");
				return ret;
			}

			gpio_set_value(exynos->usb_hs_sel, 0);
		}
	}
