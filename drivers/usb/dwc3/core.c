...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	dwc->usb3vbus_ctrl = of_get_named_gpio(dev->of_node,
				"samsung,usb3vbus_ctrl", 0);
	if (!gpio_is_valid(dwc->usb3vbus_ctrl))
		dev_info(dev, "usb3vbus_ctrl is not available\n");
	dwc->boost5v_en = of_get_named_gpio(dev->of_node,
				"samsung,boost5v_en", 0);
	if (!gpio_is_valid(dwc->boost5v_en))
		dev_info(dev, "boost5v_en is not available\n");
	...
	if (gpio_is_valid(dwc->usb3vbus_ctrl)) {
		ret = devm_gpio_request_one(dev, dwc->usb3vbus_ctrl,
						GPIOF_OUT_INIT_LOW, "dwc3_vbus_ctrl");
		if (ret) {
			dev_err(dev, "failed to request dwc3 vbus_ctrl");
		}
	}
	if (gpio_is_valid(dwc->boost5v_en)) {
		ret = devm_gpio_request_one(dev, dwc->boost5v_en,
						GPIOF_OUT_INIT_LOW, "dwc3_boost5v_en");
		if (ret) {
			dev_err(dev, "failed to request dwc3 vbus_ctrl");
		}
	}
