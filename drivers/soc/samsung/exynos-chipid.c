...
void __init exynos_chipid_early_init(struct device *dev)
{
	...
	int  hwrev0, hwrev1, hwrev2, hwrev3;
	...
	exynos_soc_info.revision = exynos_soc_info.product_id & EXYNOS_REV_MASK;
        ...
        hwrev0 = of_get_named_gpio(np, "chipid,hw_rev0", 0);
        hwrev1 = of_get_named_gpio(np, "chipid,hw_rev1", 0);
        hwrev2 = of_get_named_gpio(np, "chipid,hw_rev2", 0);
        hwrev3 = of_get_named_gpio(np, "chipid,hw_rev3", 0);

	exynos_soc_info.board_rev = gpio_get_value(hwrev1) | gpio_get_value(hwrev2) << 1 | gpio_get_value(hwrev3) << 2;
}
...
static int __init exynos_chipid_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Exynos: CPU[%s] CPU_REV[0x%x]  board rev [0.%x] Detected\n",
			product_id_to_name(exynos_soc_info.product_id),
			exynos_soc_info.revision, exynos_soc_info.board_rev);
	return 0;
free_rev:
