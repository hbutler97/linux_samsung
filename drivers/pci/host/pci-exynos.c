...
static int exynos_pcie_set_l1ss(int enable, struct pcie_port *pp)
{
#ifdef PCIE_POWER_SAVE_160930
...
static void exynos_pcie_msi_init(struct pcie_port *pp)
{
	...
	printk(">>>>>>Change MSI address to 0x%lx(0x%lx).<<<<<<\n",
			(long unsigned int)pp->msi_data, (long unsigned int)msi_target);
	if ((msi_target >> 32) != 0)
		printk("MSI allocation ERR!!!\n");
	dw_pcie_msi_init(pp);
