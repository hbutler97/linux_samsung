	...
	struct snd_pcm_hw_params *params)
{
	...
	/* Set SYSCLK FLL */
	ret = snd_soc_codec_set_pll(priv->codec, CLEARWATER_FLL1_REFCLK,
				    ARIZONA_FLL_SRC_NONE, 0, 0);
	if (ret != 0)
		dev_err(card->dev, "Failed to start SYSCLK FLL REF: %d\n", ret);

	ret = snd_soc_codec_set_pll(priv->codec, CLEARWATER_FLL1,
				    ARIZONA_CLK_SRC_MCLK1,
				    PACIFIC_MCLK_FREQ, 98304000);
	if (ret != 0)
		dev_err(card->dev, "Failed to start FLL for SYSCLK: %d\n", ret);

	/* Set SYSCLK from FLL*/
	ret = snd_soc_codec_set_sysclk(priv->codec, ARIZONA_CLK_SYSCLK,
				      ARIZONA_CLK_SRC_FLL1, 98304000, SND_SOC_CLOCK_IN);
	if (ret < 0)
	...
	ret = snd_soc_codec_set_pll(priv->codec, CLEARWATER_FLL2_REFCLK,
				    ARIZONA_FLL_SRC_MCLK1, PACIFIC_MCLK_FREQ, 98304000);
	if (ret != 0)
		dev_err(card->dev,
				"Failed to start ASYNCCLK FLL REF: %d\n", ret);

	ret = snd_soc_codec_set_pll(priv->codec, CLEARWATER_FLL2,
				    ARIZONA_FLL_SRC_AIF2BCLK, bclk, 98304000);
	if (ret != 0)
...
static struct snd_soc_card pacific_wm1840 = {
...
static int pacific_audio_probe(struct platform_device *pdev)
{
	...
	struct snd_soc_card *card = &pacific_wm1840;
