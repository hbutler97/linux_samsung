...
static int esa_ctl_nxpbdl_info(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = NXPBDL_MAX_COUNT;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = 192000; //FIXME: check this value again
	return 0;
}

static int esa_ctl_nxprvb_ctx_info(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = NXPRVB_CTX_MAX_COUNT;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = 192000; //FIXME: check this value again
	return 0;
}

static int esa_ctl_nxprvb_param_info(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = NXPRVB_PARAM_MAX_COUNT;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = 192000; //FIXME: check this value again
	return 0;
}
...
static int esa_ctl_nxpbdl_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	int i;
	int effect_val[NXPBDL_MAX_COUNT];

	pr_info("%s\n", __func__);

	for (i = 0; i < NXPBDL_MAX_COUNT; i++) {
		effect_val[i] = ucontrol->value.integer.value[i];
		pr_debug("nxpbdl: effect_val[%d] = %d", i, effect_val[i]);
	}

	esa_effect_write(NXPBUNDLE, effect_val, NXPBDL_MAX_COUNT);

	return 0;
}

static int esa_ctl_nxprvb_ctx_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	int i;
	int effect_val[NXPRVB_CTX_MAX_COUNT];

	pr_info("%s\n", __func__);

	for (i = 0; i < NXPRVB_CTX_MAX_COUNT; i++) {
		effect_val[i] = ucontrol->value.integer.value[i];
		pr_debug("nxpbdl: effect_val[%d] = %d", i, effect_val[i]);
	}

	esa_effect_write(NXPREVERB_CTX, effect_val, NXPRVB_CTX_MAX_COUNT);

	return 0;
}

static int esa_ctl_nxprvb_param_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	int i;
	int effect_val[NXPRVB_PARAM_MAX_COUNT];

	pr_info("%s\n", __func__);

	for (i = 0; i < NXPRVB_PARAM_MAX_COUNT; i++) {
		effect_val[i] = ucontrol->value.integer.value[i];
		pr_debug("nxpbdl: effect_val[%d] = %d", i, effect_val[i]);
	}

	esa_effect_write(NXPREVERB_PARAM, effect_val, NXPRVB_PARAM_MAX_COUNT);
...
static int esa_ctl_nxpbdl_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s\n", __func__);
...
static int esa_ctl_nxprvb_ctx_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s\n", __func__);
...
static int esa_ctl_nxprvb_param_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s\n", __func__);
...
static const struct snd_kcontrol_new esa_effect_controls[] = {
	...
	ESA_CTL_EQ_SWITCH("NXP BDL data", 0, \
		esa_ctl_nxpbdl_info, esa_ctl_nxpbdl_get, esa_ctl_nxpbdl_put),
	ESA_CTL_EQ_SWITCH("NXP RVB ctx data", 0, \
		esa_ctl_nxprvb_ctx_info, esa_ctl_nxprvb_ctx_get, esa_ctl_nxprvb_ctx_put),
	ESA_CTL_EQ_SWITCH("NXP RVB param data", 0, \
		esa_ctl_nxprvb_param_info, esa_ctl_nxprvb_param_get, esa_ctl_nxprvb_param_put),
};
